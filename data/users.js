...
const createUser = async (
  ...
  if (await userCollection.findOne({ email: email }))
    throw { status: 400, message: `An account with that email already exists` };
  ...
  if (!insertUserInfo.acknowledged || !insertUserInfo.insertedId)
    throw { status: 500, message: "Could not create new user" };
...
const getUserByEmail = async (userEmail) => {
  userEmail = validators.validateEmail(userEmail, "user");
  const userCollection = await users();
...
const checkUser = async (email, password) => {
  if (!email || !password)
    throw { status: 400, message: "Must provide both email and password" };
  const userCollection = await users();
