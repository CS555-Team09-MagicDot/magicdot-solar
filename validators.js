...
const validateString = (str, field) => {
  if (!str)
    throw { status: 400, message: `Must provide a string for ${field}` };
  if (typeof str !== "string")
    throw { status: 400, message: `${field} must be a string` };
  str = str.trim();
  if (str.length < 1)
    throw {
      status: 400,
...
const validateId = (id, field) => {
  ...
  if (!ObjectId.isValid(id))
    throw { status: 400, message: `Input is an invalid ID for ${field}` };
  return id;
};
...
const validateName = (name, type) => {
  ...
  if (name.length < 2)
    throw { status: 400, message: `${type} must be at least 2 characters` };
  const regex = /[^A-z\s'"]/g;
...
const validateRole = (role) => {
  ...
  if (
    ...
    throw { status: 400, message: "Input is not a valid role" };
  return role;
};
...
const validatePhone = (phone) => {
  ...
  if (phone.length !== 10)
    throw { status: 400, message: "Phone number must have 10 digits" };
  ...
  for (i = 0; i < phone.length; i++) {
    if (
      !(
        Number.isInteger(Number(phone[i])) &&
        Number(phone[i]) >= 0 &&
        Number(phone[i]) <= 9
      )
    )
  ...
  if (!parseInt(phone))
    throw { status: 400, message: "Phone must be a 10 digits number" };
  return phone;
};
...
const validateSubject = (subject) => {
  ...
  if (typeof subject !== "string")
    throw { status: 400, message: "Job subject must be a string" };
  subject = subject.trim();
  if (subject.length === 0)
    throw { status: 400, message: "Job subject cannot be empty spaces" };
...
const validateMessage = (message) => {
  ...
  if (message.length === 0)
    throw { status: 400, message: "Description cannot be empty spaces" };
  if (message.split(" ").length < 5)
    throw { status: 400, message: "Description must have at least 5 words" };
...
const validatePassword = (password) => {
  ...
  if (password.length < 8)
    throw { status: 400, message: "Password must at least 8 characters long" };
  if (password.includes(" "))
    throw { status: 400, message: "Password must not contain space" };
  if (!oneUpper.test(password))
    throw { status: 400, message: "Password must contain one upper case" };
  ...
  if (!oneNumber.test(password))
    throw { status: 400, message: "Password must contain one number" };
...
const validateDate = (date, field) => {
  ...
  if (isNaN(Date.parse(date)))
    throw { status: 400, message: `Input is not a valid date for ${field}` };
