...
router
  ...
  .get(async (req, res) => {
    try {
      if (req.session.user) {
        if (req.session.role === "sales representative")
          return res.status(200).redirect("/sales");
        else if (req.session.role === "customer")
          return res.status(200).redirect("/customer");
        else if (req.session.role === "it admin")
          return res.status(200).redirect("/it_admin");
      }
  ...
  .post(async (req, res) => {
    try {
      ...
      if (userData.role === "sales representative")
        return res.status(200).redirect("/sales");
