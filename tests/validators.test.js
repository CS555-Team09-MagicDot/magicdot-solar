...
test("Should return the processed string", () => {
  expect(validators.validateString("String Value", "A field")).toBe(
    "String Value"
  );
});
...
test("Should return role string in lowercase for valid role name", () => {
  expect(validators.validateRole("Operational Manager")).toBe(
    "operational manager"
  );
});

test("Should return email string value", () => {
  expect(validators.validateEmail("B4zvI@example.com")).toBe(
    "b4zvi@example.com"
  );
});
...
test("Should return string value of valid Message", () => {
  expect(validators.validateMessage("Test Message with five words")).toBe(
    "Test Message with five words"
  );
});
